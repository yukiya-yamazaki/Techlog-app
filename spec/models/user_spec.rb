...
describe User do
  ...
  describe '.first' do
    before do
      @user = create(:user, nickname: nickname, email: email) # 事前にユーザーを作成
      @post = create(:post, title: 'タイトル', content: '本文', user_id: @user.id) # 事前に作成したユーザーに紐づく投稿を作成
    end

    subject { described_class.first }

    it '事前に作成した通りのUserを返す' do
      expect(subject.nickname).to eq('テストユーザー')
